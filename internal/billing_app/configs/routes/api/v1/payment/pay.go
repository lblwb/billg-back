...
type BillPayGatewayService struct {
...
type PayGateWay struct {
	...
	IconVector string  `json:"icon"`
	Currency   string  `json:"curr"`
}

type PayGwBillRespDef struct {
	Url     string  `json:"url"`
	Expired string  `json:"exp"`
	Amount  float64 `json:"amount"`
}

type PayGatewayBill struct {
	Amount     float64 `json:"a"`
	CurBalance string  `json:"b"`
}

func NewBillPayGatewayService(db *database.StorageDb) *BillPayGatewayService {
	return &BillPayGatewayService{
		db:          db,
		ratesEntity: currency.NewRatesEntity(db),
	}
}

func (bpg BillPayGatewayService) encodeToString(max int) string {
...
func (bpg BillPayGatewayService) actGateway() []PayGateWay {
	gateways := []PayGateWay{{
		Name:      "e5e37fcb-9e3b-46cd-a675-7508b338d990",
		Title:     "CARD",
		MinAmount: 5,
		Currency:  "rub",
		IconVector: `<svg xmlns="http://www.w3.org/2000/svg" width="36px" height="36px"
		...
		Name:      "90c9dec2-b360-4d72-8151-0e3cb164d2ca",
		Title:     "BTC",
		MinAmount: 15,
		Currency:  "usdt",
		IconVector: `<svg width="36" height="36" viewBox="0 0 32 32" fill="none"
		...
		Name:      "10cb9327-e823-43b9-8b65-804fff9f13d9",
		Title:     "ETH",
		MinAmount: 15,
		Currency:  "usd",
		IconVector: `<svg width="20" height="36" viewBox="0 0 20 32" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
		...
		Name:      "6420dcc1-b461-422d-87ed-58ca719a4e98",
		Title:     "SBP",
		MinAmount: 10,
		Currency:  "rub",
		IconVector: `<svg width="23" height="36" viewBox="0 0 23 32" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
...
func (bpg BillPayGatewayService) findActiveGw(gwName string) PayGateWay {
	gateways := bpg.actGateway()
...
func (bpg BillPayGatewayService) getGateway(gwName string, data PayGatewayBill) (interface{}, error) {
	gwFindData := bpg.findActiveGw(gwName)
	if gwFindData.Name == gwName {
		switch gwName {
		case "e5e37fcb-9e3b-46cd-a675-7508b338d990":
			payment, err := bpg.getLavaPayment(data)
			return payment, err
		case "6420dcc1-b461-422d-87ed-58ca719a4e98":
			payment, err := bpg.getLavaPayment(data)
			return payment, err
		default:
			return nil, nil
...
func (bpg BillPayGatewayService) GetAllGateway(ctx *fiber.Ctx) error {
	gateways := bpg.actGateway()
...
func (bpg BillPayGatewayService) CreateGateway(ctx *fiber.Ctx) error {
	var gatewayName = ctx.Params("gw_name")
	var gatewayBillData PayGatewayBill
	err := ctx.BodyParser(&gatewayBillData)

	//Currency (lang) usd
	if gatewayBillData.CurBalance == "89" {
		dirCurrency, _, err := bpg.ratesEntity.GetExcRateByDirCurrency("USD")
		if err != nil {
			return ctx.JSON(fiber.Map{
				"error":     true,
				"error_msg": err,
		...
		gatewayBillData.Amount = gatewayBillData.Amount * dirCurrency.Value
	} else {
	...
	gwResult, err := bpg.getGateway(gatewayName, gatewayBillData)
	if err != nil {
		return ctx.JSON(fiber.Map{
			"error":     true,
			"error_msg": err,
...
* LAVA PAYMENT
 */
func (bpg BillPayGatewayService) getLavaPayment(data PayGatewayBill) (interface{}, error) {
	log.Println(data)
	if data.Amount > 1 {
		orderId := bpg.encodeToString(10)
		invoiceResponse, err := lava.NewClient(os.Getenv("LAVA_SHOP_ID"), os.Getenv("LAVA_SECRET_KEY")).
			CreateInvoice(data.Amount, "", orderId, 300)
		if err != nil {
			return nil, err
		}

		return PayGwBillRespDef{
			Url:     invoiceResponse.Data.URL,
			Expired: invoiceResponse.Data.Expired,
			Amount:  invoiceResponse.Data.Amount,
		}, nil
	} else {
		return nil, errors.New("not avail amount")
	}
}
