...
import (
	"backend/pkg/render/inertia-fiber/utils"
	"encoding/json"
	"fmt"
	"html/template"
	"strconv"
...
func (e *Engine) renderHTML(data *Page, c *fiber.Ctx, tmpl string, params map[string]any) error {
	...
	vals := fiber.Map{
		...
		"Vite":    utils.Vite([]string{e.config.AssetsPath + "/app.js", e.config.AssetsPath + "/Pages/" + data.Component + ".vue"}, e.config.ManifestRoot),
	}
